...
/**
 * Protempa destination for writing data to phenotype search result tables.
 * 
 * @author Nita Deshpande
 */
public class PhenotypeSearchDestination extends AbstractDestination {

    private final EurekaPhenotypeSearchConfiguration config;

    PhenotypeSearchDestination(Configuration config) {
    	if (config == null) {
            throw new IllegalArgumentException("config cannot be null");
        }
        this.config = (EurekaPhenotypeSearchConfiguration)config;
    }
    ...
    public QueryResultsHandler getQueryResultsHandler(Query query, DataSource dataSource, KnowledgeSource knowledgeSource, List<? extends ProtempaEventListener> eventListeners) throws QueryResultsHandlerInitException {
        try {
			return new PhenotypeSearchResultsHandler(query, dataSource, knowledgeSource, this.config);
		} catch (Exception e) {
