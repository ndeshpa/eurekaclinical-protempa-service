...
import edu.emory.cci.aiw.cvrg.eureka.etl.entity.PhenotypeDestinationConceptSpecEntity;
import edu.emory.cci.aiw.cvrg.eureka.etl.entity.PhenotypeDestinationModifierSpecEntity;
 ...
 */
class PhenotypeConcepts implements Concepts {
	private final FolderSpec[] folderSpecs;

	PhenotypeConcepts(List<PhenotypeDestinationConceptSpecEntity> conceptSpecs) {
		this.folderSpecs = new FolderSpec[conceptSpecs.size()];
		for (int i = 0, n = conceptSpecs.size(); i < n; i++) {
			PhenotypeDestinationConceptSpecEntity conceptSpec = conceptSpecs.get(i);
			List<PhenotypeDestinationModifierSpecEntity> modifierSpecs = conceptSpec.getModifierSpecs();
			ModifierSpec[] ms = new ModifierSpec[modifierSpecs.size()];
			for (int j = 0; j < ms.length; j++) {
				PhenotypeDestinationModifierSpecEntity mse = modifierSpecs.get(j);
