 ...
 */
public class CovidOmopDestination extends AbstractDestination {

    private final CovidOmopConfiguration config;

    CovidOmopDestination(Configuration config) {
    	if (config == null) {
            throw new IllegalArgumentException("config cannot be null");
        }
        this.config = (CovidOmopConfiguration)config;
    }
    ...
    public QueryResultsHandler getQueryResultsHandler(Query query, DataSource dataSource, KnowledgeSource knowledgeSource, List<? extends ProtempaEventListener> eventListeners) throws QueryResultsHandlerInitException {
        try {
			return new CovidOmopQueryResultsHandler(query, dataSource, knowledgeSource, this.config);
