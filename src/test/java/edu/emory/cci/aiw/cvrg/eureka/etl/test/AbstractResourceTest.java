...
public abstract class AbstractResourceTest extends JerseyTest {
    ...
    protected <E extends Object> E getJson(String path, GenericType<E> genericType) {
        return resource().path(path).accept(
                MediaType.APPLICATION_JSON).get(genericType);
    }
    
    protected <E extends Object> E getJson(String path, Class<E> cls) {
        return resource().path(path).accept(
                MediaType.APPLICATION_JSON).get(cls);
    }
